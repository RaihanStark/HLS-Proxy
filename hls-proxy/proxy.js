...
const proxy = function ({
  ...
  const regexs = {
    ...
    vod_start_at: new RegExp(
      "#vod_start(?:_prefetch_at)?=((?:\\d+:)?(?:\\d+:)?\\d+)$",
      "i"
    ),
    urls: new RegExp(
      "(^|[\\s'\"])((?:https?:/)?/)?((?:[^\\?#,/\\s'\"]*/)+?)?([^\\?#,/\\s'\"]+?)(\\.[^\\?#,/\\.\\s'\"]+(?:[\\?#][^\\s'\"]*)?)?([\\s'\"]|$)",
  ...
  const get_request_options = function (url, referer_url) {
    ...
    request_options.headers = Object.assign(
      ...
      referer_url
        ? {
            referer: referer_url,
            origin: referer_url.replace(regexs.origin, "$1"),
          }
        : {}
    );
  ...
  const should_prefetch_url = function (url) {
    ...
    if (do_prefetch) {
      ...
      if (
        hooks &&
        hooks instanceof Object &&
        hooks.prefetch &&
        typeof hooks.prefetch === "function"
      ) {
        const override_prefetch = hooks.prefetch(url);

        if (
          typeof override_prefetch === "boolean" &&
          override_prefetch !== do_prefetch
        ) {
          debug(
            3,
            "prefetch override:",
            override_prefetch ? "allow" : "deny",
            url
          );
  ...
  const modify_m3u8_content = function (m3u8_content, m3u8_url, referer_url) {
    ...
    if (debug_level >= 2) {
      m3u8_content = m3u8_content.replace(
        regexs.keys,
        function (match, head, key_url, tail) {
          debug(2, "key:", key_url);
          return match;
        }
      );
    ...
    const is_vod = cache_segments
      ? typeof vod_start_at_ms === "number" ||
        (!has_cache(m3u8_url) && regexs.vod.test(m3u8_content))
      : null;
    ...
    m3u8_content = m3u8_content.replace(
      regexs.urls,
      function (match, head, abs_path, rel_path, file_name, file_ext, tail) {
        if (
          (head === `"` || head === `'` || tail === `"` || tail === `'`) &&
          head !== tail
        )
        ...
        let matching_url;
        if (!abs_path) {
          matching_url = `${base_urls.relative}${rel_path || ""}${file_name}${
            file_ext || ""
          }`;
        } else if (abs_path[0] === "/") {
          matching_url = `${base_urls.absolute}${abs_path}${
            rel_path || ""
          }${file_name}${file_ext || ""}`;
        } else {
          matching_url = `${abs_path}${rel_path || ""}${file_name}${
            file_ext || ""
          }`;
        }
        ...
        if (
          ...
          hooks instanceof Object &&
          hooks.redirect &&
          typeof hooks.redirect === "function"
        ) {
          ...
          try {
            ...
            if (typeof matching_url !== "string")
              throw new Error("bad return value");

            if (
              matching_url.length &&
              matching_url.toLowerCase().indexOf("http") !== 0
            ) {
              matching_url =
                (matching_url[0] === "/"
                  ? base_urls.absolute
                  : base_urls.relative) + matching_url;
            }
          } catch (e) {
            matching_url = "";
          }
          ...
          if (!matching_url) {
            debug(
              3,
              "redirecting (post-hook):",
              "URL filtered, removed from manifest"
            );
            return `${head}${tail}`;
          }
        }
        ...
        if (should_prefetch_url(matching_url)) prefetch_urls.push(matching_url);

        if (vod_start_at_ms && regexs.m3u8.test(matching_url))
          matching_url += `#vod_start=${Math.floor(vod_start_at_ms / 1000)}`;

        if (referer_url) matching_url += `|${referer_url}`;

        let ts_file_ext = get_ts_file_ext(file_name, file_ext);
        let redirected_url = `${is_secure ? "https" : "http"}://${
          process.env.HOST_SERVER
        }/${base64_encode(matching_url)}${ts_file_ext || file_ext || ""}`;
    ...
    if (prefetch_urls.length) {
      if (is_vod && vod_start_at_ms) {
        ...
        const skip_segment_count = Math.floor(
          vod_start_at_ms / seg_duration_ms
        );
        ...
        debug(
          3,
          "prefetch (ignored):",
          `${skip_segment_count} URLs in m3u8 skipped to initialize vod prefetch timer from start position obtained from HLS manifest URL #hash`
        );
      }
      if (prefetch_urls.length > max_segments) {
        if (
          hooks &&
          hooks instanceof Object &&
          hooks.prefetch_segments &&
          typeof hooks.prefetch_segments === "function"
        ) {
          prefetch_urls = hooks.prefetch_segments(
            prefetch_urls,
            max_segments,
            is_vod,
            seg_duration_ms,
          ...
          if (!is_vod) {
            ...
            debug(
              3,
              "prefetch (ignored):",
              `${overflow} URLs in m3u8 skipped to prevent cache overflow`
            );
          } else {
            ...
            const is_client_paused = () => {
              const time_since_last_access =
                get_time_since_last_access(m3u8_url);
            ...
            const prefetch_next_batch = (is_cache_empty) => {
              ...
              if (!is_cache_empty && is_expired(m3u8_url)) {
                debug(
                  3,
                  "prefetch (stopped):",
                  "vod stream removed from cache due to inactivity longer than timeout; prefetch has stopped"
                );
                return;
              }
              ...
              if (!is_cache_empty && is_client_paused()) {
                debug(
                  3,
                  "prefetch (skipped):",
                  "vod stream is paused; prefetch will continue after client playback resumes"
                );
      ...
      if (socket && socket.remoteAddress) {
        ...
        if (acl_whitelist.indexOf(remoteIP) === -1) {
          ...
          debug(
            2,
            socket.remoteFamily,
            "connection blocked by ACL whitelist:",
            remoteIP
          );
        }
      }
  ...
  server.on("request", (req, res) => {
    ...
    const [url, referer_url] = (() => {
      if (!regexs.wrap.test(req.url)) return ["", ""];
    ...
    if (cache_segments && !is_m3u8) {
      ...
      if (segment && segment.length) {
        // Buffer (cached segment data)
        send_ts(segment);
        return;
      } else if (segment === false) {
        // false (prefetch is pending: add callback)
