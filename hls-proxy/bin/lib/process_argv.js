...
if (
  ...
  argv_vals["--referer"] ||
  argv_vals["--useragent"] ||
  (Array.isArray(argv_vals["--header"]) && argv_vals["--header"].length)
) {
  ...
  if (Array.isArray(argv_vals["--header"]) && argv_vals["--header"].length) {
    let split = function (str, sep) {
      ...
      if (start === -1) delete chunks[1];
      else chunks[1] = str.substr(start);
...
// =============================================================================
if (
  argv_vals["--req-secure-honor-server-cipher-order"] ||
  argv_vals["--req-secure-ciphers"] ||
  argv_vals["--req-secure-protocol"] ||
  argv_vals["--req-secure-curve"]
) {
  ...
  if (argv_vals["--req-secure-protocol"]) {
    argv_vals["--req-options"]["secureProtocol"] =
      argv_vals["--req-secure-protocol"];
  }
...
if (typeof argv_vals["--cache-key"] !== "number") argv_vals["--cache-key"] = 0;

if (typeof argv_vals["-v"] !== "number") argv_vals["-v"] = 0;
